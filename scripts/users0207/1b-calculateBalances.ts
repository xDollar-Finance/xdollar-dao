...
async function queryUserVeHndBalances() {
    ...
    let userLocks: Array<UserLock> = JSON.parse(fs.readFileSync(`./scripts/users0207/snapshots/${chainName}-locks.json`).toString());
    ...
    if (deployments.VotingEscrow) {
            ...
            if (block) {
                ...
                fs.writeFileSync(`./scripts/users0207/balances/${chainName}.json`, JSON.stringify(users, null, 4));
            }
    }
}
